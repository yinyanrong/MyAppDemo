...
//        }
        ...
        setMeasuredDimension(width,rectHeight);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(greenLine!=0){
            ...
            //绘制绿色的条
            canvas.drawRect(0,0, greenEndwidth, getMeasuredHeight(), colorPaint);
        }
        if(redLine!=0){
            colorPaint.setColor(Color.RED);
            //绘制红色的条
            canvas.drawRect(greenEndwidth, 0, greenEndwidth+ss*redLine, getMeasuredHeight(), colorPaint);

        }
        colorPaint.setColor(Color.GRAY);
        for (int i = 0; i <= showNumber; i++) {
            //绘制灰色的小圆点
            canvas.drawCircle(sumCircleRadius, getHeight() / 2, circleRadius, colorPaint);
